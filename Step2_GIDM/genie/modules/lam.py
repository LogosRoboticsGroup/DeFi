...
class UncontrolledDINOLatentActionModel(nn.Module):
    ...
    def __init__(
        ...
        if pythonpath_env:
            ...
            REPO_ROOT = Path(first_entry).parent
        else:
        ...
        # 网络加载 or 本地加载 DINOv2 模型
        ###
        self.dino_encoder = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14_reg')
        ###
        # DINO_DIR = CKPT_DIR / "dinov2"
        # DINO_CHECKPOINT = DINO_DIR / "checkpoints" / "dinov2_vitb14_reg4_pretrain.pth"
        ...
        self.text_encoder = T5EncoderModel.from_pretrained(str(T5_BASE_DIR))
        self.text_encoder.requires_grad_(False)
        self.lang_proj = nn.Linear(768, model_dim)

        # Load T5 tokenizer
        self.tokenizer = T5Tokenizer.from_pretrained(str(T5_BASE_DIR))
    ...
    def forward(self, batch: Dict) -> Dict:
        # Encode + VQ
        B, T = batch["videos"].shape[:2]  # batch["videos"] torch.Size([3, 2, 3, 224, 224])
        ...
        # Decode
        video_recon = self.decoder(video_action_patches, lang_embed.unsqueeze(1), attention_mask)  # torch.Size([3, 1, 274, 768])
