...
class Video_Former_3D(nn.Module):
    ...
    def __init__(
        ...
        self.dim = dim  # 384
        self.num_queries = num_latents  # 224
        self.num_frame = num_frame  # 16
        self.condition_dim = condition_dim  # 2560
        self.use_temporal = use_temporal  # True
        ...
        frame_seq_len = num_latents // num_frame  # 14
    ...
    def forward(self, x_f: torch.Tensor, mask: torch.BoolTensor = None, extra : torch.Tensor = None):
        ...
        batch_size, max_length, _, dim = x_f.shape  # torch.Size([28, 16, 512, 2560])

        # Mask the position embeddings for the padded frames
        time_pos_emb = (
            self.time_pos_emb[:max_length].unsqueeze(0).expand(batch_size, -1, -1, -1)
        )  # [batch_size, max_length, 1, dim] torch.Size([28, 16, 1, 384])
        if mask is not None:  # False
            time_pos_emb = time_pos_emb * mask.unsqueeze(-1).unsqueeze(-1)

        # Apply the position embeddings
        x_f = self.goal_emb(x_f)  # torch.Size([28, 16, 512, 384])
        if extra is not None:  # False
            extra = repeat(extra, 'b q d -> b T q d', T=max_length)
            x_f = torch.cat([x_f, extra],dim = 2)
        x_f = x_f + time_pos_emb  # torch.Size([28, 16, 512, 384])

        # Flatten the frames
        x_f = rearrange(x_f, 'b T n d -> (b T) n d')  # torch.Size([448(28*16), 512, 384])

        # Copy the latents for every element in the batch, this is the learnable tokens
        x = repeat(self.latents, 'T q d -> b T q d', b=batch_size)  # torch.Size([28, 16, 14, 384])
        x = rearrange(x, 'b T q d -> (b T) q d')  # torch.Size([448(28*16), 14, 384])

        # 16帧, 每帧有14个query, 用这个从图像中提取特征, 然后传递给policy head

        # Apply attention and feed forward layer
        if self.use_temporal:  # True
            for attn, Temp_attn, ffw in self.layers:
                x = x + attn(x_f, x)  # PerceiverAttentionLayer, Spatial-attn
                x = rearrange(x, '(b T) q d -> (b q) T d', b = batch_size)
                x = x + Temp_attn(x)  # Attention, Temporal-attn
        ...
        x = x.reshape(batch_size, -1 ,x.shape[1],x.shape[2])  # torch.Size([28, 16, 14, 384])
        x = rearrange(x, 'b T q d -> b (T q) d')  # torch.Size([28, 224, 384])
        assert x.shape == torch.Size([batch_size, self.num_queries, self.dim])
        norm = self.norm(x)

        return norm  # torch.Size([28, 224, 384])
