...
class Diffusion_feature_extractor(nn.Module):
    ...
    def forward(
            self,
            pixel_values: torch.Tensor,  # torch.Size([56, 1, 3, 256, 256])
            texts,
        ...
        pixel_values = rearrange(pixel_values, 'b f c h w-> (b f) c h w').to(dtype)  # torch.Size([56, 3, 256, 256])
        ...
        image_embeddings = encoder_hidden_states  # torch.Size([56, 20, 1024])
        ...
        if pixel_values.shape[-3] == 4:
            ...
            image_latents = self.pipeline._encode_vae_image(pixel_values, device, num_videos_per_prompt, False)  # 这里最后一个不一样，生成视频时候是True
        image_latents = image_latents.to(image_embeddings.dtype)  # torch.Size([56, 4, 32, 32])

        num_frames = 16
        image_latents = image_latents.unsqueeze(1).repeat(1, num_frames, 1, 1, 1)  # torch.Size([56, 16, 4, 32, 32])
        ...
        added_time_ids = self.pipeline._get_add_time_ids(
            fps,
            motion_bucket_id,
            0,  # 这里在生成视频时候是0.02，这里是0
            image_embeddings.dtype,  # 这里在生成视频时候是float32，这里是float16
            batch_size,
            num_videos_per_prompt,
            False,  # 这里在生成视频时候是True，这里是False
        )
        added_time_ids = added_time_ids.to(device)  # torch.Size([56, 3])

        self.pipeline.scheduler.set_timesteps(timestep, device=device)  # 这里生成视频时候是30，这里是20
        timesteps = self.pipeline.scheduler.timesteps  # torch.Size([20])

        num_channels_latents = self.pipeline.unet.config.in_channels  # 8
        ...
        for i, t in enumerate(timesteps):
            ...
            latent_model_input = self.pipeline.scheduler.scale_model_input(latent_model_input, t)  # torch.Size([56, 16, 4, 32, 32])

            # Concatenate image_latents over channels dimention
            latent_model_input = torch.cat([latent_model_input, image_latents], dim=2)  # torch.Size([56, 16, 8, 32, 32])

            # predict the noise residual
            feature_action, feature_pred = self.step_unet(
                latent_model_input,
                t,
                ...
                all_layer = all_layer,
                complete = complete,
            )  # torch.Size([56, 16, 2560, 16, 16])

            # if not complete:  # True
            #     break
            if i == 0:
                feature_action_out = feature_action

            # 只有一次前向过程的时候, 这个用不上
            latents = self.pipeline.scheduler.step(feature_pred, t, latents).prev_sample

        frames = self.pipeline.decode_latents(latents, num_frames, 7)
        # torch.Size([2, 16, 4, 32, 32]) -> torch.Size([2, 3, 16, 256, 256])
        
        frames = svd_tensor2vid(frames, self.pipeline.video_processor, output_type='pil')
        videos = StableVideoDiffusionPipelineOutput(frames=frames).frames

        if self.text == None:
            self.text = texts[0]
            print(f"Diffusion Extractor Text: {self.text}")
            ...
            videos = np.concatenate([video for video in videos],axis=-2).astype(np.uint8)  # (16,512,256*batch,3) (16, 256, 512, 3)
            
            save_dir = '/DeFI/ckpts/iclr_video_out'
            os.makedirs(save_dir, exist_ok=True)

            time = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
            filename = f"{save_dir}/{time}.mp4"

            writer = imageio.get_writer(filename, fps=4)
            for frame in videos:
                writer.append_data(frame)
            writer.close()
        elif self.text != texts[0]:
            self.text = texts[0]
            print(f"Diffusion Extractor Text: {self.text}")
        ...
        return feature_action_out  # torch.Size([56, 16, 2560, 16, 16])
    ...
    def step_unet(
        ...
        if not complete:
            if all_layer:
                sample_action = torch.cat(feature_list, dim=1)
                sample_action = sample_action.reshape(batch_size, num_frames, *sample_action.shape[1:])
            else:
                sample_action = sample_action.reshape(batch_size, num_frames, *sample_action.shape[1:])
            # 6. post-process
    ...
    def encode_text(self, texts, tokenizer, text_encoder, img_cond=None, img_cond_mask=None, img_encoder=None, position_encode=True, use_clip=False, max_length=20):
        ...
        with torch.no_grad():
            if use_clip:  # True
                ...
                encoder_hidden_states = outputs.last_hidden_state  # torch.Size([2, 20, 512])
                ###### will be used in the dp ##########
                # self.text_embeds = outputs.text_embeds
                ######################################
                if position_encode:  # False
