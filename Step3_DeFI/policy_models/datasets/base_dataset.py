...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.with_lang = key == "lang"  # True
        self.relative_actions = "rel_actions" in self.observation_space["actions"]  # True
        assert window_sampling_strategy in ('random', 'geometric')
        self.window_sampling_strategy = window_sampling_strategy  # 'geometric'
        self.geometric_p_value = geometric_p_value  # only needed for geomtric sampling 0.1
        self.pad = pad  # False
        self.batch_size = batch_size  # 28
        self.num_workers = num_workers
        self.min_window_size = min_window_size  # 21
        self.max_window_size = max_window_size  # 50
        self.abs_datasets_dir = datasets_dir
        self.lang_folder = lang_folder  # if self.with_lang else None 'lang_clip_resnet50'
        self.aux_lang_loss_window = aux_lang_loss_window  # 8
        assert "validation" in self.abs_datasets_dir.as_posix() or "training" in self.abs_datasets_dir.as_posix()
        self.validation = "validation" in self.abs_datasets_dir.as_posix()  # False for train, True for val
    ...
    def __getitem__(self, idx: Union[int, Tuple[int, int]]) -> Dict:
        ...
        if isinstance(idx, int):
            ...
            elif self.min_window_size < self.max_window_size:  # True
    ...
    def _get_window_size(self, idx: int) -> int:
        ...
        Returns:
            Window size.
        """
        window_diff = self.max_window_size - self.min_window_size  # 29
        if len(self.episode_lookup) <= idx + window_diff:
            # last episode
            max_window = self.min_window_size + len(self.episode_lookup) - idx - 1
        elif self.episode_lookup[idx + window_diff] != self.episode_lookup[idx] + window_diff:  # True
            ...
            max_window = min(self.max_window_size, (self.min_window_size + steps_to_next_episode - 1))  # 28
        else:
        ...
        if self.validation:
            ...
            if self.window_sampling_strategy == 'geometric':  # True
                p = self.geometric_p_value  # Choose a suitable value for p 0.1
                while True:
